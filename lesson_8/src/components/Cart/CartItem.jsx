...
import { removeGoodFromCart } from "../../redux/Slices/cartSlice";
import { useState } from "react";


function CartItem() {
    const cart = useSelector((state) => state.cart.cartGoods);
    ...
    return (
        <>
            {cart.length === 0 ? "Добавьте свой первый товар в корзину!" : cart.map((cartItem) => {
                return (
                    <div className="product-selection__card-first" key={cartItem.id}>
                        <img src={cartItem.photo} alt="Изображение товара" />
                        <div className="product-selection__card-info">
                            <div className="product-selection__header">
                                <h2 className="product-selection__title">{cartItem.title}</h2>
                                <button className="product-selection__title__button" onClick={() => dispatch(removeGoodFromCart(cartItem))}><svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <p className="product-selection__price">Price: {cartItem.price} $</p>
                            <p className="product-selection__size">Size: {cartItem.size}</p>
